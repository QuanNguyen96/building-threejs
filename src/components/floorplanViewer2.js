...
const initFunc = forwardRef((props, ref) => {
  ...
  useEffect(() => {
    ...
    function onMouseUp(event) {
      ...
      setArrayObjectSelected(allSelected)
      // const allObjects = scene.children.filter(o => o.isMesh); // hoặc chỉ các box
      // const selected = selectionBox.select(allObjects);
      // // console.log("selected=", selected)

      //   selected.forEach(obj => {
      //     obj.material.color.set(0xff0000); // feedback màu đỏ
      //   });
      //   console.log("selected", selected)

      //   // setSelectedMeshes(selected); // lưu lại selected
    }
